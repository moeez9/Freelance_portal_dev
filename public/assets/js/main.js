...
$(document).on('click', '.btn_view_job', function () {
    let jobId = $(this).data('id');

    $.get(`/employer/jobs/${jobId}/view`, function (job) {

        $('#jobTitle').text(job.title);
        $('#jobType').text(job.type);
        $('#jobCategory').text(job.category);
        $('#jobSalary').text(job.salary);
        $('#jobDeadline').text(job.deadline);
        $('#jobDescription').html(job.description);

        // modal open
        $('[data-type="modal_view_job"]').addClass('active');
    });
});
        ...
        $('#form-review .form').on('submit', function (e) {
            if (selectedRating === 0) {
                ...
                if (window.flashModal) {
                    window.flashModal({ type: 'error', message: 'Please select your rating before submit the comment.' });
                }
            } else {
